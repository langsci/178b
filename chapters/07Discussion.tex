...
Overall,
I showed that correlated but distinct features affect the choice of particles, \isi{word order}, and intonation in spoken Japanese.
...
Namely, \isi{topic} markers are sensitive to
a and b in \ref{Dis:Ex:ISFeatures}.
The marker \ci{toiuno-wa} codes elements referring to an entity in evoked status in the \isi{hearer}'s mind.
The marker \ci{wa} codes elements referring to an entity in \isi{inferable} status,
...
I also showed that the \isi{topic} markers are partially sensitive to grammatical function (f in \ref{Dis:Ex:ISFeatures});
when the clause has two evoked arguments, A and P,
A is more likely to be coded by \isi{topic} markers (in this case, \ci{wa}),
rather than P.
...
I proposed that three inter-related principles, repeated here as \ref{Disc:oldnewprinciple}, \ref{Disc:IScontinuityP}, and \ref{Disc:PerFirstPrinciple} are working to determine \isi{word order} of spoken Japanese.
...
the \isi{hearer} is assumed to be able to identify the referent
because it is strongly evoked.
They are produced for some intonational or \isi{interactional} reasons
as has been discussed in \ref{WO:PostP:Motivations}.

In Chapter \ref{Intonation},
I discussed intonation.
I showed that
evoked, \isi{inferable}, declining, and unused topics tend to be produced in an \isi{intonation unit} separately from the predicate,
while strongly evoked topics tend to be produced in an \isi{intonation unit} together with the predicate.
On the other hand,
the \isi{broad focus} tends to appear in an \isi{intonation unit} with the predicate
...
a \isi{topic} appears in an \isi{intonation contour} and a focus appears in another \isi{intonation contour},
...
whereas when the \isi{activation cost} of a \isi{topic} is low,
...
As summarized above,
there is no single feature (such as \isi{topic} or focus) which determines
the choice of particles, \isi{word order}, and intonation;
...
\ci{ko} in \ili{Hindi} codes definite or \isi{animate} \isi{direct object};
there is no single feature that determines the use of \ci{ko}.
Citing \cite{poppe70}, he discusses another example from \ili{Mongolian}.
According to Poppe, the \isi{accusative} suffix \ci{-iig} only attaches to certain kinds of direct objects.
...
The distinction between the so-called \isi{accusative marker} \ci{o} and the zero particles in Japanese is similar to the use (or non-use) of this suffix \ci{-iig} in \ili{Mongolian}.
%in the sense that they are sensitive to multiple features.
The choice between \ci{o} and the zero particles is reported to be determined by \isi{definiteness}, \isi{animacy}, and \isi{word order}.
Definite or \isi{animate} objects are more likely to be coded by \ci{o}
...
choices of particles, \isi{word order}, and intonation in spoken Japanese.
For example,
...
If a single feature ``\isi{topic}'' or ``focus'' determines the choice of \isi{word order} and particles,
it is expected, for example, that
all clause-initial elements are coded by \isi{topic} markers
because both clause-initial elements and those coded by \isi{topic} markers are topics.
However, this is not the case,
as shown in \S \ref{WO:ClauseInit:Ident:Topic}.
Although clause-initial elements tend to be coded by \isi{topic} markers,
not all clause-initial elements are coded by \isi{topic} markers.
This is because \isi{word order} and \isi{topic} coding are sensitive to different features, while both of them are sensitive to topichood and focushood; clause-initial elements are sensitive to identifiability,
whereas \isi{topic} markers are sensitive to \isi{activation status} of the referent in question.
...
The theory of competing motivations and correlating features of \isi{topic} and focus \ref{Dis:Ex:ISFeatures} predicts that
...
This study showed a variety of statistical tendencies of particle choice, \isi{word order}, and intonation in Japanese.
...
strongly evoked elements in spoken Japanese can appear immediately after the predicate, with a single \isi{intonation contour} with the predicate.
...
an \isi{affix}, which is coreferential with the subject or the object,
typically attaches to the \isi{verb}.
...
Also, preposed objects are attested in \ili{Swahili},
and they have an agreement relationship with a \isi{verb} \isi{affix} in a way similar to subject agreement.
...
while the object-agreement originates from afterthought-\isi{topic} constructions like \ref{Disc:HardConst:Integrated:Ex:AT},
...
On the other hand,
incorporated objects appear after the \isi{verb} (before the subject),
from which one can see \isi{noun incorporation}.
Unlike typical \isi{noun incorporation},
...
However Southern Tiwa is like \ili{Onondaga} and \ili{Niuean} discussed above
in the sense that
...
This is parallel with \ili{Onondaga}, as shown by the contrast between \Next and \NNext.
...
This applies no matter whether the language is basically of the ergative, \isi{accusative}, or agent/patient type.
...
i.e.,  \isi{animate}, human, and/or definite Ps,
are overtly coded,
...
i.e., languages that do not allow unestablished topics to appear together with the predicate or \isi{main clause}.
...
the connection between the subject (A and S) and \isi{topic} is so strong
...
subjects cannot be focus in situ (See \citeA{downinghyman16} and works cited therein for the summary of \isi{information structure} in Bantu languages).
...
\chd{Also in \ili{French}, which is a SV(P) language,
...
\ex.
 \ag. Dans cet immeuble n'habitent pas \EM{des} \EM{ouvriers} \EM{fran\c{c}ais}, mais des ouvriers espagnols. \\
      ...
      `In this building, \ili{French} workers do not live, but \ili{Spanish} workers.'
