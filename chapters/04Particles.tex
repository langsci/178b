...
so-called \isi{topic} particles coding different kinds of topics (\S \ref{TopPar})
and
so-called case particles coding different kinds of foci and grammatical functions (\S \ref{CasePar}).
Table \ref{ParInfoStatusT} summarizes kinds of so-called \isi{topic} particles and case particles
coding topics and focus in different statuses of the given-new taxonomy.
The \isi{activation status} is also specified in the table to show the correspondence,
...
The morpheme \ab{cop} indicates \isi{copula}.
...
I argue that these tables are a kind of \isi{semantic map} \cite{croft01,haspelmath03}.
By arguing that Tables \ref{ParInfoStatusT} and \ref{OvertZeroCaseParT1} are examples of semantic maps,
I postulate that
the scales of the given-new taxonomy (the column) and the \isi{topic} vs.\ focus distinction (the row) in Table \ref{ParInfoStatusT} and
...
\ex. \label{SemanticMapHypIS}
 ...
 Since the scales of given-new taxonomy and the \isi{topic} vs.\ focus distinction in Table \ref{ParInfoStatusT} and
...
The statuses in the given-new taxonomy have corresponding activation statuses in the \isi{hearer}'s mind assumed by the speaker.
I propose that \isi{inferable} and declining elements and unused and brand-new elements are in different activation statuses in the assumed \isi{hearer}'s mind.

Table \ref{ParInfoStatusCTT} and Figure \ref{ParInfoStatusCTF} show the distributions of elements in different \isi{information status} coded by different particles in our corpus.
...
Note that ``\isi{anaphoric}'' in the present work just means ``the element in question has the co-referential \isi{antecedent}'' and ``non-\isi{anaphoric}'' means ``it does not.''
Elements with bridging antecedents are included in ``non-\isi{anaphoric}.''
...
The model with the effects of particles, \isi{word order}, and intonation is significantly different from the model without each of those effects (likelihood ratio test, $p<0.001$ a model without particles, $p<0.01$ one without \isi{word order}, and $p<0.05$ without intonation).%
...
the least-squares mean of the odds ratio of \isi{anaphoric} elements coded by \ci{ga} is significantly less than the least-squares mean of the odds ratio of those coded by \ci{o};
in other words, \isi{anaphoric} elements are more likely to be coded by \ci{o} than by \ci{ga}.
Similarly, \isi{anaphoric} elements are more likely to be coded by \ci{wa} than by \ci{ga}, by \ci{ni}, and by \ci{mo}.
...
\ci{toiuno-wa} codes as high a ratio of \isi{anaphoric} elements as \ci{wa} does.
...
the referent of non-\isi{anaphoric} elements coded by \ci{toiuno-wa} has been introduced implicitly in the previous contexts.
%Note that
...
There are not enough examples for the \isi{copula} followed by \ci{ga} or \ci{kedo} (7 examples)
...
the persistent elements are mentioned at least once in the following \isi{discourse} after it is mentioned,
...
The model with the effects of particles, \isi{word order}, and intonation is significantly different from the model without either of the effects of particles and \isi{word order} (likelihood ratio test, $p<0.001$ a model without particles, $p<0.01$ the model without \isi{word order}).
...
For example,
as in \Next,
it is unacceptable for \isi{topic} markers to code brand new elements \ci{oozei-no hito} `many people' out of the blue.
...
Therefore, \isi{topic} markers in Japanese are sensitive to the given-new taxonomy rather than \isi{definiteness} and identifiability.%
 \footnote{
...
an element coded by a \isi{zero particle} ({\O}) that precedes other arguments and is uttered in a coherent \isi{intonation contour}
...
availability in the universe of \isi{discourse}.
...
In \Next,
\ci{supootu-kansen} `sport watching' is non-\isi{anaphoric}
...
Therefore, ``human beings'' are always available as \isi{topic}.
...
\exg.\label{Par:Toiunowa:Ex:Hawaii}teema-wa hawai-too-no sizen-no subarasisa-to tabi-no tanosisa-nituite-desu \\
   ...
   `The \isi{topic} (of this talk) is about the splendor of Hawaii nature and fun of traveling.'
...
it is not always available as \isi{topic}
...
The climate in Iran is neither implicitly introduced nor available as universal \isi{topic}.
...
Apparently, it does not simply correspond to the distance between an element and its \isi{antecedent},
but the intervention of (an)other \isi{topic}(s) seems to be more relevant.
For example,
...
It is fair to assume that the \isi{topic} `job' is intervened by another \isi{topic} `fame'.
When the element `job' is retrieved as a current \isi{topic} in line g,
it is coded by a \isi{copula} followed by \ci{keredomo} `though',
...
\ci{Toiuno-wa} cannot code elements that have not been established as \isi{topic}.
In \Next,
although `tea time' is introduced in line b,
it does not appear to be established enough as \isi{topic},
...
and there are few things the speaker can assume to be shared with the \isi{hearer}(s).
...
Whereas the stage-level \isi{predication} \Next[a] does not allow \ci{toiuno-wa},
the individual-level \isi{predication} \Next[b] does allow \ci{toiuno-wa}.
%
\ex.
...
borne in the \isi{hearer}'s mind at the time of \isi{utterance},
...
Unlike \ci{toiuno-wa},
\ci{wa} also codes \isi{inferable} elements extensively.
In \Next,
...
\ci{Wa} sometimes forces the \isi{hearer} to accept the assumption that the \isi{hearer} has already been thinking about the \ci{wa}-coded referent;
I call this {accommodation}.
In \Next, which immediately follows \Last,
\ci{wa} which codes \ci{gyappu} `gap' in line c
forces the \isi{hearer} to accept the assumption that s/he expected the speaker to talk about the gap between the expectation and the reality.
%
...
they do not force the \isi{hearer} to accommodate the assumption that s/he has already been thinking about them.

What can be \isi{inferable} depends on the culture.
In Japanese culture,
...
which the \isi{hearer}(s) can easily expect the speaker to start talking about out-of-the-blue.
...
%%% Reference
is a special case for \ci{wa} coding \isi{inferable} elements.
...
the contrastively \ci{wa}-coded elements are not necessarily \isi{anaphoric} (given),
...
`job' is intervened by another \isi{topic} `fame'.
...
they argue that \ci{ga} newly introduces topics in the beginning of a \isi{discourse}.
...
as shown in \Next,
where `ice cream' is assumed to be \isi{inferable} because they are talking about the things in the fridge and both of them know that there was ice cream there.
%
\ex. Context:
...
where the second \isi{topic} (job) is considered to be declining.
%
\ex.\label{sigoto2}
...
I categorize this example as a marginal case of \isi{inferable}
and \ci{kedo} functions to upgrade the referent to the \isi{topic} status.

Unused elements can be coded by \ci{kedo},
as shown in \Next.
In \Next,
it is assumed that speaker Y and \isi{hearer} H share a particular ice cream
...
Thus all the examples of topics coded by \ci{kedo} above might also be the predicates of \isi{copula} clauses.
%
...
First,
it is actually impossible to ``recover'' the subject of alleged \isi{copula} clauses in topic-coding \ci{kedo},
...
\ci{ga}-coded subordinate clauses state background of the \isi{main clause} and
that this use of subordinate \ci{ga} grammaticalizes into \isi{topic} marker.
...
Also, they precede other arguments and are followed by the \isi{accentual-phrase boundary}.
On the other hand,
...
they are foci in the sense that it is newly introduced into the \isi{discourse}.
...
This suggests that the speaker assesses through \ci{kedo} the \isi{hearer}'s state of knowledge,
...
The findings of \isi{topic} codings are summarized in Table \ref{ParInfoStatusT},
...
the speaker assumes that the \isi{hearer}(s) do(es) not know Hawaii
...
the \ci{toiuno-wa}-coded elements tend to be evoked at the time of \isi{utterance}
...
The discussion in \S \ref{kedo} supports the previous observation of this \isi{topic} expression;
the expression is used to newly introduce topics in the beginning of a \isi{discourse} or a paragraph \cite{koide84,takahashi99}.
...
While \isi{topic} markers code topics of different statuses in the given-new taxonomy as discussed in the previous section,
...
In \Next,
where the speaker's neck is presupposed to be at issue at the time of \isi{utterance} \Next[A],
only \ci{wa}-coding is natural,
\chd{although zero \isi{pronoun} is more natural in this context}.
%
...
In \Next,
on the other hand,
where the speaker's neck is not presupposed to be at issue at the time of \isi{utterance} \Next[A],
...
S indicates the only argument of \isi{intransitive} clause, and
...
Note that Table \ref{OvertZeroCaseParT} is also a kind of \isi{semantic map};
a scale of \isi{agentivity} on the one hand
and that of \isi{contrastiveness} on the other.
\chd{Here I categorize \isi{argument focus} together with \isi{contrastive focus}
to refer to ``\isi{contrastive focus}''
because, as long as \ci{ga}/\ci{o} vs.~zero-coding is concerned, argument and \isi{contrastive focus} do not differ from each other;
...
the category that each the marker codes should map onto a connected region in \isi{conceptual space}.
In the following sections,
...
to code \isi{non-contrastive focus} A.
%
...
Contrastive focus (or \isi{argument focus}) A is only naturally coded by \ci{ga}
...
when S is \isi{non-contrastive focus},
...
In \ili{Hindi},
for example,
the postposition \ci{ko} codes definite or \isi{animate} (especially human) \isi{direct object},
while other kinds of direct objects tend to be zero-coded.
There is no simple correlation of \ci{ko} with either \isi{animate} or definite \isi{direct object}.
In the following example \Next,
where \ab{do} stands for `\isi{direct object} marker',
sometimes \ci{ko} codes \isi{animate} elements, as in \Next[a] but sometimes not, as in \Next[c],
...
\ci{ko} codes complex features of \isi{animacy}, \isi{definiteness}, and \isi{direct object}.
\ex.
...
First,
\ci{ga}, but not {\O$_{f}$}, codes \isi{inanimate} A.
For example, in \Next,
...
To summarize,
\ci{ga} sometimes codes \isi{animate} patient S like \ref{ExAnimateS},
sometimes codes non-\isi{animate} agent like \ref{ExInanimateA},
sometimes codes non-\isi{nominative} \isi{inanimate} focus elements, as in \ref{ExNon-ArgFocus} to \Last, and,
probably more frequently,
it codes elements with complex features of \isi{agentivity}, \isi{animacy}, and focus.
...
while \isi{contrastive focus} P is only naturally coded by \ci{o}.
...
\isi{non-contrastive focus} A can only naturally be coded by \ci{ga},
...
This study revealed the distributions of case particles and the zero particles in term of \isi{information structure}.
...
In this section,
I will briefly discuss the relationships between grammatical function and \isi{information structure}.
This is associated with the issue that has long been discussed in the literature: the connection between \isi{topic} and subject
...
84.7\% of \ci{wa} attaching nouns code so-called subjects (A and S in my terms, \isi{nominative case} in her terms) in novels and essays,
...
So-called subjects have tspecial status in the \isi{discourse};
they are interpreted as definite in the \isi{discourse}
...
This observation suggests that subjects without topic-marking still function like \isi{topic} markers.
...
In fact, the majority of \isi{anaphoric} Ps are still coded by \ci{o},
...
although non-\isi{anaphoric} Ss (most of which are in fact \isi{inferable}) are remarkably more often coded by \ci{wa} than others.

I propose the hierarchy \Next for topic-coding.
The given elements higher in this hierarchy are more likely to be coded by \isi{topic} markers.
%
...
\ili{German}, for example, has the principle that only allows the \isi{verb} in the second position within a clause, as exemplified in \Next[a-d].
However, the \isi{detached topic} constituents that appear at the beginning are not counted as the first constituent of the clause.
%rather they are outside of the clause.
As exemplified in \Next[e],
the \isi{verb} \ci{isst} `ate' appears in the second position assuming that the preceding \ci{den} `it' is in the first position,
which indicates that
the \isi{detached topic} \ci{den Apfel} is not counted as the first constituent in the clause.
...
and the case for A, S, and P coded by \isi{topic} markers are not overtly expressed
...
the \isi{topic} is not necessarily an argument;
...
through linguistic expressions (such as particles, \isi{word order}, and intonation) and other means (such as gesture, background knowledge, sequence of conversation, etc.).
...
the particles code elements with features that can be mapped onto a \isi{conceptual space}.
...
hence, the \isi{inferable} status is closer to the evoked status, and the declining status is closer to unused in the \isi{conceptual space}.
...
The element `cause' is \isi{inferable} because the disease has been already introduced and the cause of the disease can be considered to be part of the knowledge of getting a disease.
%
\ex.
...
Thus, I argue that the adjacency of \isi{inferable} and evoked statuses and that of declining and unused statuses are cognitively motivated and I argue that this is universal.
...
Figure \ref{DistExpTypeF2} shows the time difference (\isi{anaphoric distance}) on a logarithmic scale between when the \isi{first mora} of the element in question is produced and when that of its \isi{antecedent} is produced.
...
Since overt pronouns coded by the \isi{topic} markers are as strongly evoked as zero pronouns,
...
focus A is the most rare in natural occurring \isi{discourse}
...
This chapter discussed the distributions of so-called \isi{topic} marker and case markers in Japanese.
I argued that
...
Another possibility is to investigate conversations and study the elements that the \isi{hearer} actually responds to.
