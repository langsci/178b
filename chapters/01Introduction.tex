...
First,
I will investigate the relationship between \isi{information structure} and linguistic forms in spoken Japanese.
Second,
I propose a method to investigate the relations between \isi{information structure} and linguistic forms in any language using corpora.

Speakers of Japanese, like speakers of other languages,
...
can be interpreted to be either definite or \isi{indefinite},
while that followed by \ci{wa} in \Next[b]
can only be interpreted to be definite;
from \Next[b] we can infer that the speaker assumes the \isi{hearer} already knows about the dog.
%
\ex.
...
the choices of particles, \isi{word order}, and intonation
...
Also, I suppose that \isi{information structure} should be analyzed at the surface level rather than at the level of underlying semantics (or logical form).

There are two roots of studies on \isi{information structure}
(see \citeA{kruijff-korbayovasteedman03} for a useful survey).
One arose from studies on definite and \isi{indefinite} descriptions by \citeA{russell05} and \citeA{strawson50,strawson64}.
These studies triggered the discussion on \isi{presupposition} and assertion,
...
the so-called \isi{topic} particle \ci{wa} in Japanese,
often as opposed to the \isi{case particle} \ci{ga},
...
inherit both of the traditions of \isi{information structure}:
the logical and the functional traditions.
Other corpus-oriented studies such as \citeA{hajicovaetal00},
annotating \ili{Czech}, are based on the Prague School theory.
There are also questionnaires for eliciting expressions related to \isi{information structure} cross-linguistically \cite{skopeteasetal06}.
...
In this study,
I focus on the question of how the usage related to \isi{information structure} affects linguistic form in Japanese.
...
But many of the studies employ multiple features to decide the \isi{information structure} of a sentence.
For example,
\citeA{gotzeetal07},
in annotating ``\isi{aboutness} \isi{topic}'',
...
rather than annotating homogeneous ``\isi{topic}'' and ``focus''.
I consider a \isi{topic} as a cluster of features of presupposed, evoked, definite, specific, \isi{animate}, etc.,
and a focus as a cluster of features of asserted, brand-new, \isi{indefinite}, non-specific, \isi{inanimate}, etc.
