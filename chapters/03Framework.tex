...
%types of information structure in a sentence (\ref{FrameworkIS}) and
...
The corpus is called \ci{the Corpus of Spontaneous Japanese} (CSJ),
...
nouns such as \ci{hon} `book' accompany \ci{no} to modify another nouns and \ci{da} for predication. %(58-59)
...
I call this type of test \ci{no}-test
 (see also the lie-test in \citeA[p.~39]{erteschik-shir07}).
...
This suggests that
once B says \ci{no}, he must provide an alternative to a focus (as long as s/he knows).
I am inclined to label \ci{ROB} in \Last[B2] as focus
and think that the existence of examples like \LLast[B] does not spoil the \ci{no}-test.
...
I call this test \ci{aha}-test.
\ci{Aha}-test is a natural consequence of the fact that
...
because the referent that the preposed element refer to is presupposed to be shared between the speaker and the hearer as \ci{nezumi} `mouse' in \Next,
...
As shown in \Last,
(part of) the focus \ci{a teacher} can be negated smoothly,
...
there is no explicit topic and all the arguments (e.g., \ci{the children} and \ci{school} in \Last[A]) are part of focus.
...
\ci{kyoo} `today' might function as topic in the sense that
...
and refer to them as \EM{broad focus structure}.

In \EM{the argument-focus structure},
elements other than predicates are focused.
This is exemplified in \Next[A] for English
and \NNext[A] for Japanese.
This structure is sometimes referred to as \EM{narrow focus structure}
...
\ci{nezumi} `mouse' in \ref{FrameworkExMouse} is topic because the the characteristics are along the line with topic correlation features in \ref{ISFeatures}
...
\ex.\label{tomodati} \a.[Q:] What did you guys do today?
	\bg.[A:] [watasi-wa]$_{T}$ [tomodati-to resutoran-de supagetii tabe-ta]$_{F}$-yo \\
			\ab{1}.\ab{sg}-\ci{wa} friend-with restaurant-\ab{loc} spagetti eat-\ab{past}-\ab{fp} \\
...
which makes it possible for \ci{mangoo} to appear clause-initially as will be discussed in Chapter \ref{WordOrder}.
...
I include this type of example like \ci{mangoo} in unused,
extending the term ``unused'' in \citeA{prince81}.

However,
some indefinite referents are difficult to be topics than others.
For example, expressions such as \ci{dareka} `somebody' and \ci{oozee-no hito} `many people' are difficult to be topics
...
In \Next,
the element \ci{kuzira} corresponds to a generic referent as topic.
%
...
\ci{wa} codes the element whose referent is assumed to be active or semi-acitve inferable in the hearer's mind and
...
prompts to omit the element \ci{koinu} `puppy'
...
the question prompts to \ci{koinu} `puppy' as answer.
...
Relevant variables will be explained in each chapter.
...
In Japanese,
pronouns such as \ci{watashi} `\ab{1}\ab{sg}', \ci{anata} `\ab{2}\ab{sg}', and \ci{kare} `\ab{3}\ab{sg}' are rare;
...
An expression that has an antecedent is called \EM{anaphoric} element,
while an expression that does not have an antecedent is called \EM{non-anaphoric} element.
...
still, \ci{zoo} `elephant' looks like part of the clause and needs some label,
...
%The tool used for statistical analyses and graphics throughout the thesis
%is R.%
%	\footnote{
