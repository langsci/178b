...
While many researchers hypothesize multiple layers of \isi{information structure},
I instead suppose a flat layer of \isi{information structure} with multiple features.
...
The notion of \isi{topic} is controversial, and the history is complicated.
I classify these complicated notions into several representative categories in the following subsections.
Before the overview, I first introduce the definition of \isi{topic} in this study to make the discussion clear.
...
Since I assume that \isi{information structure} is a cognitive notion, I define the \isi{topic} from a cognitive standpoint.
The definition of \isi{topic} is stated in \Next.
%
\ex. \label{BackDefTopic} Topic is a \isi{discourse} element that the speaker assumes or presupposes to be shared (known or taken for granted) and uncontroversial in a given sentence both by the speaker and the \isi{hearer}.
...
topics defined in \Last include unused, declining (to be discussed below), \isi{inferable}, and evoked elements \cite[\S 4.4.2]{lambrecht94}.%
 \footnote{
 Inferable elements are further divided into containing and non-containing \isi{inferable} elements, and
...
A new element refers to an entity the speaker first introduces into the \isi{discourse};
...
A declining element refers to an entity which has been mentioned a while ago but is assumed to be declining in the \isi{hearer}'s mind because it has not been referred to for a while.
Declining elements are assumed to be in \isi{semi-active state} in terms of \citeA{chafe87,chafe94}.
The referents of declining elements are in \isi{semi-active state} especially through ``deactivation from an earlier active state'' \cite[29]{chafe87}.
Chafe's concept of semi-active also includes \isi{inferable} entities.
Since I want to distinguish \isi{inferable} from declining,
I introduce a new term.

Note that the condition where the speaker assumes the element to be shared is a necessary but not a sufficient condition of \isi{topic};
if the element in question is a \isi{topic}, a \isi{topic} is assumed by the speaker to be shared with the \isi{hearer},
...
Also note that the definition of \isi{topic} in \ref{BackDefTopic} includes heterogeneous elements in \Last.
...
One of the representative definitions of \isi{topic} is that
\isi{topic} is what the sentence is about.
...
if the assertion is understood as intending to expand our knowledge of this \isi{topic}'' \cite[59]{reinhart81}.%
  \footnote{
  Although Reinhart's idea of the definition of \isi{topic} is basically from Strawson,
...
an expression X is a \isi{topic} if it is felicitously paraphrased as \{\ci{as for}/\ci{regarding}\} X (p.~63, see also \citeA{kuno72,kuno76,gundel74}).
...
For example, \ci{as for this book} in \Next is awkward even though this is clearly a \isi{topic}.
...
The context set of a given \isi{discourse} at a given point is a set of propositions that both the speaker and the \isi{hearer} have accepted to be true at this point \cite{stalnaker78}.
...
the definition \ref{BackDefTopic} identifies a \isi{topic}
better than the \isi{aboutness} test,
although \isi{aboutness} captures some aspects of our intuition about topics.
...
Following Prince,
I use the term ``evoked information'' to indicate the referent that has been mentioned in the previous \isi{discourse} or has been physically present in the speaker's and the \isi{hearer}'s attention
...
As it is well known, evoked elements can be focus instead of \isi{topic} as shown in \Next[B].
%
...
For example, the whole \isi{utterance} in \Next[a-d] can be the answer to a question ``what happened?'',
...
In \Next,
objects are topics.
The \isi{information structure} is annotated by the current author.
...
\citeA{chomsky65} and \citeA{halliday67} characterize the \isi{topic} as the sentence-initial element
(more recently, see \citeA{hajicovaetal00}).
To define the \isi{topic} in terms of linguistic form pre-empts the goal of this study:
i.e., to figure out the association between information structures (\isi{topic} and focus) and linguistic forms (particles, \isi{word order}, and intonation).
...
Like \isi{topic}, focus is also a controversial notion and the literature disagrees on the definition as well as the properties of focus.
...
Since I try to capture the phenomena of \isi{information structure} in a single layer,
...
I believe the statement that the speaker ``wants the \isi{hearer} to learn the relation of the \isi{presupposition} to the focus'' in \ref{BackFocDef} is essentially the same as the definition of a comment in \citeA{gundel88},
...
The interpretation of \isi{information structure} is of the current author
...
Focus is news (or newsworthy in \citeA{mithun95}) for the \isi{hearer} and can be repeated as what s/he learned from the current \isi{utterance}.
For example, in \Next,
...
I argue that new elements that have been known to the \isi{hearer} before the \isi{utterance}, i.e., ``unused'' in terms of \citeA{prince81}, can be either \isi{topic} or foci.
...
associate focushood with \isi{contrastiveness} (frequently accompanied with \isi{pitch peak}).
...
for our assumptions;
whereas we have assumed that \isi{topic} and focus are mutually exclusive,
there can be both \isi{contrastive topic} and \isi{contrastive focus}
...
one has to admit that \isi{contrastive topic} is both \isi{topic} and focus.
...
these particles are obligatory for \isi{contrastiveness},
...
\chd{Some studies assume that focus is a \isi{pitch peak}.
...
the association between a \isi{pitch peak} and focus
...
with the only exception of \ili{Hixkaryana} (see also the references in her work and \citeA{buring07}).%
\footnote{
...
however,
I do not employ the definition of focus as \isi{pitch peak}
...
the definition of focus as \isi{pitch peak} spoils the goal of our study.
Moreover,
I will argue in Chapter \ref{Intonation} that 
elements other than focus are given \isi{pitch peak}.
...
Therefore, regarding focus as elements with \isi{pitch peak} causes great confusion.
...
\isi{adverbial} subordinators appear after the verbs,
...
The features of Japanese mosst relevant in this study are the order of the subject, the object, and the \isi{verb} and the order of nouns and particles.
...
The particles \ci{ga} and \ci{o}, which follow nouns, are considered to be a \isi{nominative} particle and an \isi{accusative} particle respectively in written Japanese,
...
most literature agrees that \ci{wa} is a \isi{topic} marker (no matter what it means),
...
The \isi{pitch accent} plays an important role.
The systems of \isi{pitch accent} vary among Japanese dialects,
...
In addition to phonemes and \isi{pitch} accents,
...
case and \isi{adverbial} particles.
Case particles such as \ci{ga} and \ci{o}
code grammatical relations of nouns.
For example, in \Next,
\ci{ga}, following a noun \ci{taroo}, codes \isi{nominative case},
whereas \ci{o}, following a noun \ci{hon} `book', codes \isi{accusative} case.
%
...
The \isi{adverbial particle} discussed in this study is \ci{wa}.%
 ...
 they are translated like adverbs in \ili{English},
 which is part of the reason why they are called ``\isi{adverbial}'' particles.
 }
\ci{Wa} can replace \ci{ga} and \ci{o} and change the noun into ``\isi{topic}''.
...
It has some important characteristics in addition to coding \isi{nominative case};
...
the associations between \isi{information structure} and predicate types,
...
\ci{ga} functions as a \isi{focus marker}
...
However, it is too extreme to claim that all kinds of \ci{ga} do not code \isi{nominative}.
...
It is clear that \ci{ga} sometimes codes \isi{nominative}, sometimes codes a focus, and sometimes codes both.
...
whether \ci{ga} codes a focus or \isi{nominative case}.
...
and codes the patient-like argument in a \isi{transitive clause} \cite[e.g.,][]{yamada36,shibatani90}.
In this section,
...
issues of \isi{information structure}.
...
because most of the time \ci{wa} codes so-called \isi{nominative case} in place of \ci{ga}.
...
is that \ci{wa} is a \isi{topic} marker,
...
Moreover,
as will be discussed in Chapter \ref{Particles},
78 (41.1\%) out of 190 cases of \ci{wa} codes new (non-\isi{anaphoric}) information, i.e., nouns without antecedents in the previous contexts. 
...
\citeA{kuno73} distinguishes \ci{wa} coding given (in his term, \isi{anaphoric}) information
from that coding contrastive information.
He argues that the contrastive \ci{wa} can code new (in his term, ``non-\isi{anaphoric}'') information
...
The contrastive \ci{wa} is typically accompanied by high \isi{pitch}.
...
based on the categorical vs.~\isi{thetic} judgements.
According to Kuroda,
...
Property \isi{predication} states a property of a referent \cite{masuoka87,masuoka08}.
The property is unbounded by space or time.
Masuoka states that property \isi{predication} corresponds to
individual-level \isi{predication} proposed in \citeA{carlson77}.%
 ...
 which are typically considered to be stage-level \isi{predication} rather than
 individual-level \isi{predication}.
 \ex.
...
\Next exemplifies property \isi{predication}.
...
On the other hand,
event \isi{predication} describes an event bound by time and space like \Next.
%
\ex. A child smiled.  \hfill{(op.cit.: 5)}

This corresponds to stage-level \isi{predication} in \citeA{carlson77}.

To see that \ci{toiuno-wa} is a marker only for property \isi{predication},
...
and will not go into detail about \isi{predication} types.
...
they newly introduce topics in the beginning of a \isi{discourse} or a paragraph, or
they are used to state different aspects of the current \isi{topic}
...
as shown in \Last[a];
nouns must be followed by the \isi{copula} (\ci{desu}) as shown in \Last[a] and \Next[a].
On the other hand,
the so-called \isi{case marker} \ci{ga} can directly follow nouns
as shown in \Next[b].
%
\ex.
...
Also note that \ci{ga} and \ci{kedo} as \isi{topic} markers are different from
conjunctive \ci{ga} and \ci{kedo}.
Conjunctive \ci{ga} and \ci{kedo} by definition follow clauses
instead of phrases;
on the other hand,
the \isi{topic} marker \ci{ga} and \ci{kedo} cannot follow clauses.
...
Although I employ the symbol {\O} for a \isi{zero particle} and
...
which is also considered to be in \isi{narrow focus} and
...
The subjects of a stage-level predicate or an \isi{unaccusative} predicate can be naturally zero-coded
...
the distinction between the \isi{topic} vs.~focus zero particles.
%
%\ex.
...
(e.g., plausibility of [grammatical-function]-assignment given the subject, object, and \isi{verb})'' (p.~863, see also \citeA{kurumadajaeger15}).
For example,
...
the zero particles code information the \isi{hearer} expects to hear.
As shown in the contrast between \Next and \NNext,
the \isi{zero particle} (as well as \ci{ga} in this case) can naturally code \ci{basu} `bus' \Next
...
\isi{discourse} structures affect the selection of \ci{wa} vs.~{\O}.
...
which implies that the zero particles appear at major \isi{discourse} boundaries.
For now,
...
While Japanese basic \isi{word order} is APV (or SOV in more popular terminology),
...
\isi{word order} variations of subjects, objects, datives, and possibly other arguments,
...
Functional linguists and, more recently, \isi{interactional} linguists are interested in post-predicate constructions,
...
Instead of \isi{word order} variations, they concentrate on \isi{affix} ordering and dependency relations (see e.g., \citeA{saeki98}).

I outline previous studies on basic \isi{word order} and other variations in the following sections.
...
Another important factor that affects \isi{word order} is the weight of NP.
...
the implicatures of some \isi{postposed construction} are dramatically different from the corresponding pre-pred\-i\-cate constructions.%
...
the \isi{postposed element} uttered within the same \isi{intonation contour} with the predicate (single-contour type) and
...
A smaller RD indicates that the referent has been previously mentioned relatively recently and hence the \isi{activation cost} is low;
...
\citeA{tanaka05} found that the preferred second part begins with the predicate, followed by NPs (and other adverbs and \isi{adverbial} clauses),
...
In Chapter \ref{WordOrder},
I will investigate these three kinds of NPs in terms of \isi{information structure},
...
The discussion of the basic \isi{word order} implies that
Ps most frequently appear pre-predicatively and that this is the basic order.
Therefore,
I assume that almost all theories assume that Ps appear pre-predicatively as basic \isi{word order} and
...
the argument of an \isi{unergative} \isi{verb} \ci{otoko-no ko} `boy' in \NNext appears in the same position as the subject (A) of a \isi{transitive verb} \ci{kodomo} `child' in \Next,
whereas the argument of an \isi{unaccusative} \isi{verb} \ci{ki-no eda} `tree branch' in \ref{ExKageyamaUnacc} appears in the same position as the object (P) of a \isi{transitive verb} \ci{ki-no eda} in \Next.
%
...
the argument of \isi{unaccusative} verbs and the object (P) of \isi{transitive} verbs
structurally appear closer to the \isi{verb};
i.e., they appear pre-predicatively since Japanese is basically a \isi{verb-final language}.
...
Ps, the arguments of the \isi{unaccusative} verbs, and foci appear pre-predicatively.
...
Here I outline studies on associations between intonation and some functions including \isi{information structure}.
...
although there are many interesting interactions between intonation and \isi{discourse} structure \cite[e.g.,][]{nakajimaallen93,vendittiswerts96,muraiyamashita99,koisoetal03,okuboetal03,koisoishimoto12}.
I focus on 
...
The definition of an \isi{intonation unit} makes use of a labeling system for Japanese prosodic information called X-JToBI,
...
which itself is modified from ToBI (Tones and Break Indices), a labeling system for \ili{English} prosody \cite{silvermanetal92,pitrellietal94,beckmanelam97}.%
...
I introduce an example of an \isi{intonational-phrase boundary} (the label \code{3}),
which is the most relevant to our study.
Figure \ref{BIexF} shows the \isi{pitch contour} of the \isi{utterance} \Next.
%
...
the \isi{pitch range} of the second \isi{accentual phrase} is smaller than the first one;
...
Also, whereas prominence marking, down-stepping, and boundary \isi{pitch} movements are more popular topics than intonation units,
...
``sequences with focus on the noun almost always had an \isi{intermediate phrase} [i.e., \isi{intonational phrase}] boundary between the \isi{adjective} and the noun[...] an \isi{intermediate phrase} boundary blocks catathesis [i.e., \isi{downstepping}]'',
through production experiments,
where subjects were asked to produce a sequence of an \isi{adjective} and a noun with different focus positions.
...
they conclude that their generalization applies to both accented and unaccented words.%
 ...
 from a syntagmatic perspective, the focus receives higher \isi{pitch} than the preceding phrase, which indicates that \isi{downstepping} is blocked.
 From a paradigmatic perspective, on the other hand,
 he had to conclude that \isi{downstepping} is not blocked before the focus.
...
This has been investigated based on \ili{English}, \ili{German} and \ili{Dutch}
...
%
\citeA{kori11} investigated intonation of broad and \isi{narrow focus} and
reports that, by default,
only the first word receives \isi{pitch peak} and the following word is
suppressed,
...
while \citeA{chafe87,chafe94} reports that an \ili{English} IU often corresponds to a clause.
...
The IU \Next[a] ends with a final \isi{vowel} lengthening,
...
Among foci, most of the studies (except for those on focus projection) concentrate on \isi{narrow focus} rather than \isi{broad focus}.
Moreover, almost all of them are experimental studies rather than
corpus studies.
On the other hand, I focus here on
the difference between broad foci and topics in \isi{spontaneous speech},
although I also employ a \isi{production experiment}.
...
an \isi{intonation unit} corresponds to a unit of \isi{information structure} such as \isi{topic} and focus,
...
subjects perceive the voice as though two people were overlapping with each other where there are \isi{pitch} resets and there are supposed to be pauses.
According to her,
...
Therefore, I assume that pauses correlate with \isi{pitch reset}.
